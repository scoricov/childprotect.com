...
Returned body contains list of tokens. Each token record contains an MD5 hash of submitted file, it's size and the date when it was submitted to ChildProtect.
...
* Getting tokens submitted by you
...
* Access Key ID Ñ It is the access key id of the identity you are using to send your request.
* Signature Ñ Each request must contain a valid request signature, or the request is rejected. A request signature is calculated using your Secret Access Key, which is a shared secret known only to you and ChildProtect.
* Date Ñ Each request must contain the date and time the request was created, represented as a string in UTC.
...
Examples of the <b>Date</b> header:
{{{
